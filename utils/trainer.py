        ...
        with output(initial_len=11, interval=0) as output_lines:
            for epoch in range(1, self.num_epochs+1):
                ...
                if epoch % evaluate_epoch == 0 and val_loader is not None:
                    ...
                    val_result_rounded = {k:round(v,4) for k, v in val_result.items()}
                    
                    # 仅保留指定的指标进行打印
                    keys_to_keep = ['classification_loss', 'fn', 'fp', 'aupr', 'auc', 'tn', 'tp']
                    val_result_filtered = {k: v for k, v in val_result_rounded.items() if k in keys_to_keep}
                    val_str =  f'Validation Results: ' + json.dumps(val_result_filtered, indent=4, sort_keys=True)
                ...
                test_result_rounded = {k:round(v,4) for k, v in test_result.items()}
                # 仅保留指定的指标进行打印
                keys_to_keep = ['classification_loss', 'fn', 'fp', 'aupr', 'auc', 'tn', 'tp']
                test_result_filtered = {k: v for k, v in test_result_rounded.items() if k in keys_to_keep}
                test_str = f'Test Results: ' + json.dumps(test_result_filtered, indent=4, sort_keys=True)
                output_lines[0] = ' '*30
                output_lines[1] = ' '*30
                output_lines[2] = '-'*40 
