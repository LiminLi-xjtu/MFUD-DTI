...
from sklearn.metrics import average_precision_score, roc_auc_score, f1_score, recall_score, precision_score,  matthews_corrcoef, accuracy_score, confusion_matrix
import numpy as np
...
def evaluate_cls(Y,P,threshold=0.5):
    ...
    if len(Y) == 0:
        return {k: 0.0 for k in ['auc', 'aupr', 'f1', 'recall', 'precision', 'accuracy', 'mcc', 'tn', 'fp', 'fn', 'tp']}

    predicted_label = P > threshold
    tn, fp, fn, tp = confusion_matrix(Y, predicted_label).ravel()
    return {
        ...
        'mcc': float(matthews_corrcoef(Y, predicted_label)),
        'tn': float(tn),
        'fp': float(fp),
        'fn': float(fn),
        'tp': float(tp)
...
def evaluate_mcls(Y,P):
    ...
    try:
        auc = roc_auc_score(onehot_y, P, average='macro',multi_class='ovo')
        aupr = multiclass_ap(onehot_y, P, n_classes=nclass)
    except:
        auc = -999
        aupr = -999
    ...
    multi_result = {
        'multiclass_auc': float(auc),
        'multiclass_aupr': float(aupr),
