...
from models.layers import BANLayer, MLP, AtomEncoder, Drug_GLA_GCN, Protein_GLA_GCN, DrugProteinConv, PosLinear, GCNCluster, SAGECluster, SGCluster, APPNPCluster, dropout_edge
...
class net(torch.nn.Module):
    def __init__(self,
                 # MOLECULE
                 mol_in_channels=43,  prot_in_channels=33, prot_evo_channels=640,
                 hidden_channels=200,
                 num_layers=2,
                 # interaction
                 total_layer=1,                
        ...
        for idx in range(total_layer):
            self.mol_convs.append(Drug_GLA_GCN(hidden_channels, num_layers, num_heads=1, 
                 alpha=0.5, dropout=0.1, use_bn=True, use_residual=True, use_weight=True, use_graph=True, use_act=False, graph_weight=0.8, aggregate='add'
            ))

            self.prot_convs.append(Protein_GLA_GCN(hidden_channels, num_layers, num_heads=1, 
                 alpha=0.5, dropout=0.1, use_bn=True, use_residual=True, use_weight=True, use_graph=True, use_act=False, graph_weight=0.8, aggregate='add'
            ))
        ...
        self.bcn = weight_norm(
            BANLayer(v_dim=hidden_channels, q_dim=hidden_channels, h_dim=2*hidden_channels, h_out=2),
            name='h_mat', dim=None)
        ...
        # self.latent_model = LatentModel(d_model) 
    ...
    def forward(self,
        ...
        # mol_prot_feat_att = torch.cat([mol_pool_feat, prot_pool_feat], dim=-1)
        
        # 双线性
        atom_bx_w, atom_mask_w = to_dense_batch(atom_x * atom_score, mol_batch)
        residue_bx_w, residue_mask_w = to_dense_batch(residue_x * residue_score, prot_batch)
        atom_bx, atom_mask = to_dense_batch(atom_x , mol_batch)
        residue_bx, residue_mask = to_dense_batch(residue_x , prot_batch)
        
        atom_bx = 0.8 * atom_bx_w + 0.2 * atom_bx
        residue_bx = 0.8 * residue_bx_w + 0.2 * residue_bx
        
        
        # mol_prot_feat_bcn, attention_matrix = self.bcn(atom_bx * atom_mask, residue_bx * residue_mask)
        mol_prot_feat_bcn, attention_matrix = self.bcn(atom_bx,  residue_bx)
        mol_prot_feat =  mol_prot_feat_bcn 
    ...
    def configure_optimizers(self, weight_decay, learning_rate, betas, eps, amsgrad):
        ...
        for mn, m in self.named_modules():
            for pn, p in m.named_parameters():
                ...
                # weights of blacklist modules will NOT be weight decayed
                    no_decay.add(fpn)
                elif pn.endswith('weight_g') or pn.endswith('weight_v') or pn.endswith('h_mat_g') or pn.endswith('h_mat_v'):
                    decay.add(fpn)
                elif pn.endswith('weight') and isinstance(m, torch.nn.BatchNorm1d):
                    decay.add(fpn)
