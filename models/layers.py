...
from models.gla_gcn import GLA_GCN
import torch
...
class BANLayer(nn.Module):
    ...
    def reset_parameters(self):
        ...
        if self.h_out <= self.c:
            fan_in = self.h_dim * self.k
            nn.init.xavier_normal_(self.h_mat, gain=nn.init.calculate_gain('relu'))
            nn.init.xavier_normal_(self.h_bias, gain=nn.init.calculate_gain('relu'))
        else:
            # 重置h_net的参数
...
class FCNet(nn.Module):
    """Simplified form of non-linear fully connect network similar to MLP"""
...
class Drug_GLA_GCN(nn.Module):
    def __init__(self, hidden_channels, num_layers=2, num_heads=1, 
        ...
        super(Drug_GLA_GCN, self).__init__()

        self.bond_encoder = torch.nn.Embedding(5, hidden_channels)

        self.atom_conv = GLA_GCN(in_channels=hidden_channels, hidden_channels=hidden_channels, out_channels=hidden_channels, num_layers=2, num_heads=num_heads, 
                 alpha=0.5, dropout=0.5, use_bn=True, use_residual=True, use_weight=True, use_graph=True, use_act=False, graph_weight=0.8, aggregate='add')
    ...
    def forward(self, atom_x, bond_x, atom_edge_index):
        ...
        edge_weight = torch.mean(bond_x, dim=1)
        atom_x = atom_in + F.relu(self.atom_norm(self.atom_conv(atom_x, atom_edge_index, edge_weight)))
        atom_x = F.dropout(atom_x, self.dropout, training=self.training)

        return atom_x


class Protein_GLA_GCN(nn.Module):
    def __init__(self, hidden_channels, num_layers=2, num_heads=1, 
                 alpha=0.5, dropout=0.1, use_bn=True, use_residual=True, use_weight=True, use_graph=True, use_act=False, graph_weight=0.8, aggregate='add'):
        super(Protein_GLA_GCN, self).__init__()

        self.conv = GLA_GCN(in_channels=hidden_channels, hidden_channels=hidden_channels, out_channels=hidden_channels, num_layers=2, num_heads=num_heads, 
                 alpha=0.5, dropout=0.1, use_bn=True, use_residual=True, use_weight=True, use_graph=True, use_act=False, graph_weight=0.8, aggregate='add')
    ...
    def forward(self, x, prot_edge_index, prot_edge_attr):
        x_in = x
        prot_edge_attr = torch.mean(prot_edge_attr, dim=1)
